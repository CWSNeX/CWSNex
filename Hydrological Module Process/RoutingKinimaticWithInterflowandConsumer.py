...
# K is the Number  of dx (sections)
k = 8
# -------------------------------------------------------------------------

# Where we wanna calculate Y and Discharge(Where there is interaction with another systems)
disConsumer = [100,500,1400,2000,0,0,0,0]

# Where we have Interflow in watershed
disInter = [50,600,1000,1500,0,0,0,0]
#--------------------------------------------------------------------------
...
for i in range(0,k-1):
    if disInter[i]< disConsumer[i] :
        dx.append(disInter[i])
        dx.append(disConsumer[i])
    elif disConsumer[i] < disInter[i]:
         dx.append(disConsumer[i])
         dx.append(disInter[i])
...
for i in range(0,k-1):
        # ii indexes just call dis consumer and Inter
    if disInter[i] > disConsumer[i] and disInter[i] < disConsumer[i+1] :
